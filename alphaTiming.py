...
plt.errorbar(day, thin_runs, yerr=[np.sqrt(i) for i in thin_runs], fmt="o", capsize=3, label = "Thin Sample", color="blue")
plt.fill_between(day, thin_avg_count-np.sqrt(thin_avg_count), thin_avg_count+np.sqrt(thin_avg_count), alpha=0.3, color="blue")
plt.axhline(thin_avg_count, color="blue")

plt.errorbar(day, med_runs, yerr=[np.sqrt(i) for i in med_runs], fmt="o", capsize=3, label = "Med Sample", color="green")
plt.fill_between(day, med_avg_count-np.sqrt(med_avg_count), med_avg_count+np.sqrt(med_avg_count), alpha=0.3, color="green")
plt.axhline(med_avg_count, color="green")

plt.errorbar(day, thick_runs, yerr=[np.sqrt(i) for i in thick_runs], fmt="o", capsize=3, label = "Thick Sample", color="red")
plt.fill_between(day, thick_avg_count-np.sqrt(thick_avg_count), thick_avg_count+np.sqrt(thick_avg_count), alpha=0.3, color="red")
plt.axhline(thick_avg_count, color="red")
